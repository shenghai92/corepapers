...
> = {
  ...
  thesis: {
    title: "Thesis Statement Examples and Placement: Build an Arguable Claim",
    desc: "Learn where a thesis statement usually goes in an essay, then build a specific, arguable, evidence-led claim with a fictional learning example, placement check, and revision workflow.",
    eyebrow: "Research foundations",
    h2: "Place a thesis where readers can find the paper’s controlling claim",
    intro: "In many short academic essays, a thesis appears near the end of the introduction—often the first paragraph—after readers have enough context to understand the claim. That is a useful default, not a universal rule: longer papers, discipline-specific genres, proposals, reports, reflections, and assignments may use a purpose statement, question, multi-sentence claim, or a different reader path. Follow the assignment and make the location match the argument you can support.",
    checklist: ["Does the assignment need an arguable thesis, a purpose statement, a research question, a proposal claim, or another controlling move?", "Can readers encounter enough relevant context, question, or problem before the claim without delaying it unnecessarily?", "Does the statement make a claim rather than only name a topic, and is the claim specific enough for the assignment length and available evidence?", "Could a reasonable reader ask for evidence or offer a different interpretation?", "Does the rest of the paper’s structure help demonstrate the claim, and does the final draft still place it where readers can find it?"],
    example: "Fictional learning example: This fictional learning example is invented for practice; it is not a real essay, source, claim, evidence record, or thesis to submit. A broad topic such as student planning can become an arguable fictional claim: ‘In a fictional first-year seminar context, regular weekly planning may support assignment completion by helping students make deadlines visible.’ For a short fictional argument essay, the writer would place the claim near the end of the fictional introduction after briefly framing the local question. The claim is scoped, cautious, and opens a path for evidence rather than presenting a universal fact.",
        ...
        keywords={
          kind === "sources"
            ...
            : kind === "questions"
              ...
              : kind === "paragraphs" ? "academic paragraph structure, topic sentence evidence explanation" : kind === "thesis" ? "thesis statement examples, where does thesis statement go in essay, thesis statement placement, academic thesis statement, arguable claim" : "academic argument evidence example, claim evidence explanation"
        }
        canonical={canonical}
      />
            ...
            <>
              <section className="mt-8 p-7 bg-white border border-border rounded-2xl">
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-4">
                  {[['1. TASK', 'Confirm the assignment and reader', 'Check whether the paper calls for analysis, interpretation, comparison, evaluation, explanation, proposal, or another controlling move; match the thesis form to the actual genre and reader.'], ['2. QUESTION', 'Frame a problem worth answering', 'Turn the topic into a focused analytical or normative question, then identify what is at stake for a reader rather than assuming the subject alone creates an argument.'], ['3. EVIDENCE', 'Study the available material', 'Collect and organize verified sources, data, observations, or textual details before deciding what a paper can responsibly claim.'], ['4. CLAIM TYPE', 'Choose the kind of claim', 'Distinguish interpretive, causal, evaluative, definitional, or policy claims. Different claims require different reasons, criteria, evidence, and counterpoints.'], ['5. WORKING CLAIM', 'State a provisional answer', 'Draft a central interpretation or position in your own words. A working thesis can change as research reveals a stronger relationship, alternative, or limitation.'], ['6. PLACEMENT', 'Choose the reader path before the claim', 'For many short academic essays, place the thesis near the end of the introduction once readers have necessary context and question. Do not treat one location as universal: a longer, discipline-specific, proposal, report, or reflective genre may need another controlling statement or reader path.'], ['7. CALIBRATION + SUPPORT', 'Set scope and map the evidence path', 'Name the relevant population, text, setting, time, condition, or reason; use qualifiers when evidence does not justify a universal claim. Check that planned sections can provide analysis, evidence, explanation, counterpoint, or criteria a thoughtful reader needs.'], ['8. REVERSE CHECK', 'Revise placement against the completed paper', 'Re-read the prompt, thesis, introduction, evidence map, and draft together. Change the claim, location, scope, or body when they no longer match rather than defending an early sentence.']].map(([step, title, copy]) => <article key={step} className="rounded-xl bg-muted/50 p-5"><p className="text-xs font-sans font-semibold text-primary mb-2">{step}</p><h3 className="font-serif text-xl text-slate-purple mb-2">{title}</h3><p className="text-sm font-sans text-muted-foreground leading-relaxed">{copy}</p></article>)}
                </div>
              </section>
              ...
              <section className="mt-6 grid lg:grid-cols-2 gap-6">
                ...
                <article className="p-7 bg-white border border-border rounded-2xl">
                  ...
                  <ul className="space-y-3 text-sm font-sans text-muted-foreground leading-relaxed">
                    ...
                    <li className="flex gap-2"><CheckCircle2 size={16} className="text-emerald-500 shrink-0 mt-0.5" />In many short academic essays, a thesis appears near the end of the introduction—often the first paragraph—after essential context and the question or problem. Longer and discipline-specific genres may use another location or form; the assignment controls.</li>
                  </ul>
                </article>
              </section>

              <section className="mt-6 p-7 bg-white border border-border rounded-2xl">
                <h2 className="font-serif text-3xl text-slate-purple mb-3">Test placement and revise rather than defend the first draft</h2>
                <p className="text-sm font-sans text-muted-foreground leading-relaxed"><a className="text-primary underline underline-offset-4" href="https://writingcenter.unc.edu/tips-and-tools/thesis-statements/" target="_blank" rel="noreferrer">UNC Writing Center</a> explains that a thesis is usually near the beginning of a paper—most often at the end of its first paragraph—and acts as an argument map, while also emphasizing that a working thesis can change as evidence and drafting develop. <a className="text-primary underline underline-offset-4" href="https://writingcenter.fas.harvard.edu/thesis" target="_blank" rel="noreferrer">Harvard College Writing Center</a> advises that a thesis should appear early in an academic essay, be arguable, and fit the evidence and scope available. <a className="text-primary underline underline-offset-4" href="https://owl.purdue.edu/owl/general_writing/academic_writing/establishing_arguments/index.html" target="_blank" rel="noreferrer">Purdue OWL</a> distinguishes debatable, narrow claims from broad statements that cannot be adequately supported. Ask whether your reader can find the answer after enough context, why it matters, and how the planned evidence will develop it; follow any different assignment or disciplinary convention.</p>
              </section>
              ...
              <section className="mt-6 grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <Link href="/introduction-section-example-research-paper" className="block"><article className="h-full p-5 bg-white border border-border rounded-xl hover:border-primary/30"><h2 className="font-serif text-xl text-slate-purple mb-2">Place the claim in an Introduction</h2><p className="text-sm font-sans text-muted-foreground">Give readers essential context and a focused problem, then locate the thesis where the assignment&apos;s reader path requires it.</p></article></Link>
          ...
          {kind === "sources" && (
            <>
              <section className="mt-8 p-7 bg-white border border-border rounded-2xl">
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-4">
                  {[['1. TASK ROLE', 'Start with the project need', 'Check research question, genre, reader, scope, date range, and the role a source would play: background, primary material, context, evidence, counterargument, method, or interpretation.'], ['2. IDENTITY AND VERSION', 'Identify creator, venue, and record', 'Locate author or organization, expertise, affiliations, publication venue, purpose, audience, publication date, version or revision history, and original page or publication record.'], ['3. CLAIM AND EVIDENCE', 'Read beyond the title', 'Inspect the main claim, methods, data, citations, omissions, sample or setting, and whether important information leads to original sources rather than an untraceable summary.'], ['4. PERSPECTIVE AND COVERAGE', 'Locate the viewpoint and reach', 'Ask whose interests, assumptions, population, alternatives, terms, and evidence are visible or absent; perspective can be valuable without being universal or automatically invalid.'], ['5. CROSS-CHECK', 'Test key material laterally', 'Compare central claims, dates, definitions, numbers, quotations, and evidence paths with other appropriate sources; a polished design, familiar name, domain, or peer-review label is not a complete verdict.'], ['6. PROJECT USE', 'Decide what the source can do', 'Record whether the item can responsibly provide background, a stakeholder view, a primary artifact, a claim to assess, limited evidence, or a source trail—and what it cannot establish.'], ['7. CITATION BOUNDARY', 'Keep source use traceable', 'Preserve original wording and context, distinguish the source’s ideas from your analysis, record exact publication details, and never invent author, date, DOI, finding, or reference.'], ['8. REVERSE CHECK', 'Reopen the decision before drafting', 'Check task fit, identity, version, evidence, viewpoint, limits, corroboration, intended use, note accuracy, and citation record again when the source enters your paper.']].map(([step, title, copy]) => <article key={step} className="rounded-xl bg-muted/50 p-5"><p className="text-xs font-sans font-semibold text-primary mb-2">{step}</p><h3 className="font-serif text-xl text-slate-purple mb-2">{title}</h3><p className="text-sm font-sans text-muted-foreground leading-relaxed">{copy}</p></article>)}
                </div>
              </section>
              ...
              <section className="mt-6 grid lg:grid-cols-2 gap-6">
                <article className="p-7 bg-hero-gradient border border-border rounded-2xl">
                  ...
                  <p className="text-sm font-sans text-muted-foreground leading-relaxed"><strong className="text-foreground">This fictional learning example is invented for practice; the organization, report, author, funding, evidence, review, source, finding, citation, and use decision are fictional and cannot be submitted as a real source evaluation.</strong> A student finds an invented nonprofit report arguing that late library hours would improve student wellbeing. Instead of accepting or discarding it because of the organization&apos;s viewpoint, the student checks who funded it, what population and data it used, whether the report cites accessible evidence, and whether independent studies address the same outcome. The report might help explain one stakeholder perspective, but it cannot by itself establish a general causal conclusion.</p>
                </article>
                <article className="p-7 bg-white border border-border rounded-2xl">
              ...
              <section className="mt-6 grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <Link href="/academic-integrity-and-source-use" className="block"><article className="h-full p-5 bg-white border border-border rounded-xl hover:border-primary/30"><h2 className="font-serif text-xl text-slate-purple mb-2">Keep evaluation records traceable</h2><p className="text-sm font-sans text-muted-foreground">Separate source claims, notes, quotation or paraphrase, your assessment, publication record, and in-text attribution.</p></article></Link>
                <Link href="/citations" className="block"><article className="h-full p-5 bg-white border border-border rounded-xl hover:border-primary/30"><h2 className="font-serif text-xl text-slate-purple mb-2">Verify citation details</h2><p className="text-sm font-sans text-muted-foreground">Use a citation tool only after confirming the original version, author or organization, date, title, container, and locator.</p></article></Link>
                <Link href="/academic-argument-evidence" className="block"><article className="h-full p-5 bg-white border border-border rounded-xl hover:border-primary/30"><h2 className="font-serif text-xl text-slate-purple mb-2">Match source to claim</h2><p className="text-sm font-sans text-muted-foreground">Test whether a source’s method, scope, and evidence can responsibly support, qualify, or challenge a specific argument.</p></article></Link>
              </section>
            </>
          )}
          {kind === "questions" && (
          ...
          {kind === "evidence" && (
            <>
              <section className="mt-8 p-7 bg-white border border-border rounded-2xl">
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  {[['1. TASK AND READER', 'Identify the argumentative job', 'Check what the assignment, discipline, genre, and reader require: interpretation, evaluation, explanation, definition, comparison, policy, or another claim type.'], ['2. QUESTION', 'Name the issue and stakes', 'Frame the question, problem, or disagreement the argument addresses so readers can see why a focused answer is worth considering.'], ['3. CLAIM', 'Say what you can defend', 'Make a focused, arguable point that answers the question rather than naming a topic, repeating a source, or announcing a conclusion evidence cannot support.'], ['4. EVIDENCE ROLE', 'Choose support that fits', 'Use relevant data, examples, source details, or findings that your discipline treats as appropriate support; identify whether each item supports a premise, offers context, tests an alternative, or limits reach.'], ['5. SOURCE BOUNDARY', 'Make attribution and scope visible', 'Distinguish another author’s claim, evidence, wording, or method from your own reasoning. Check that each source speaks to the population, setting, period, and claim you ask it to support.'], ['6. REASONING', 'Explain the connection', 'Show how evidence supports, complicates, or limits the claim and why the inference is warranted. A quotation, number, citation, or source list does not explain itself.'], ['7. COUNTERARGUMENT AND LIMIT', 'Engage the strongest fair challenge', 'State why a reasonable reader could disagree, represent a significant alternative fairly, then refute, qualify, concede, or revise the claim when the objection changes what can be defended.'], ['8. REVERSE CHECK', 'Test the whole argument chain', 'Re-read task, question, thesis, premises, evidence, citations, reasoning, counterargument, and conclusion together; remove irrelevant support and correct overstatement before submission.']].map(([step, title, copy]) => <article key={step} className="rounded-xl bg-muted/50 p-5"><p className="text-xs font-sans font-semibold text-primary mb-2">{step}</p><h3 className="font-serif text-xl text-slate-purple mb-2">{title}</h3><p className="text-sm font-sans text-muted-foreground leading-relaxed">{copy}</p></article>)}
                </div>
              </section>
              ...
              <section className="mt-6 grid lg:grid-cols-2 gap-6">
                <article className="p-7 bg-hero-gradient border border-border rounded-2xl">
                  ...
                  <p className="text-sm font-sans text-muted-foreground leading-relaxed"><strong className="text-foreground">This fictional learning example is invented for practice; all studies, authors, results, citations, questions, evidence, counterarguments, reasoning, and conclusions here are invented. They are not real evidence and must not be submitted or cited.</strong> A fictional paper claims that visible assignment deadlines may support more consistent planning among first-year students. It then cites two fictional observational studies with similar patterns. The analysis explains that the studies support a limited association in their reported settings, while their self-reported cross-sectional designs do not show that deadline visibility caused the change. The paragraph therefore uses a cautious implication rather than a causal recommendation.</p>
                </article>
                <article className="p-7 bg-white border border-border rounded-2xl">
              ...
              <section className="mt-6 grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <Link href="/literature-review-synthesis-matrix" className="block"><article className="h-full p-5 bg-white border border-border rounded-xl hover:border-primary/30"><h2 className="font-serif text-xl text-slate-purple mb-2">Compare evidence across sources</h2><p className="text-sm font-sans text-muted-foreground">Map themes, methods, agreements, differences, and limits before claiming a cross-source pattern.</p></article></Link>
                <Link href="/academic-paraphrasing-tool-for-esl-students" className="block"><article className="h-full p-5 bg-white border border-border rounded-xl hover:border-primary/30"><h2 className="font-serif text-xl text-slate-purple mb-2">Integrate a source responsibly</h2><p className="text-sm font-sans text-muted-foreground">Preserve verified meaning and attribution, then add your own analysis rather than cosmetic rewriting.</p></article></Link>
                ...
                <Link href="/thesis-statement-examples" className="block"><article className="h-full p-5 bg-white border border-border rounded-xl hover:border-primary/30"><h2 className="font-serif text-xl text-slate-purple mb-2">Refine the central claim</h2><p className="text-sm font-sans text-muted-foreground">Match a working thesis to the question, available evidence, scope, and serious alternatives.</p></article></Link>
                <Link href="/academic-paragraph-structure" className="block"><article className="h-full p-5 bg-white border border-border rounded-xl hover:border-primary/30"><h2 className="font-serif text-xl text-slate-purple mb-2">Build one reasoning unit</h2><p className="text-sm font-sans text-muted-foreground">Turn a premise and its evidence into a paragraph with clear attribution, explanation, limits, and connection.</p></article></Link>
                <Link href="/academic-integrity-and-source-use" className="block"><article className="h-full p-5 bg-white border border-border rounded-xl hover:border-primary/30"><h2 className="font-serif text-xl text-slate-purple mb-2">Keep argument sources traceable</h2><p className="text-sm font-sans text-muted-foreground">Check original evidence, claim scope, paraphrase or quotation, source role, and in-text attribution.</p></article></Link>
                <Link href="/citations" className="block"><article className="h-full p-5 bg-white border border-border rounded-xl hover:border-primary/30"><h2 className="font-serif text-xl text-slate-purple mb-2">Verify evidence citations</h2><p className="text-sm font-sans text-muted-foreground">Format only after verifying the original source record and the relationship it has to your claim.</p></article></Link>
              </section>
            </>
          )}
          <section className="mt-6 grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
